...
//when I'm picking a new room I want to make sure the other one couldn't do better
const computeTargetRoomScore = (rooms: Rooms, path: Path, bestRoomPaths: Record<string, number>, room: string, forHuman: boolean): number => {
    let currentTravelTime = forHuman ? path.yourTravelTime : path.eleTravelTime
    let targetRoom = forHuman ? path.yourTargetRoom : path.eleTargetRoom
    let additionalTravelTime = bestRoomPaths[`${targetRoom},${room}`]
    return (path.time - currentTravelTime - additionalTravelTime) * rooms[room].rate
}

const findBestPath2 = (rooms: Rooms): number => {

    let eleVisitedRooms: string[] = []
    let yourVisitedRooms: string[] = []
    ...
    const walk = (): void => {
        ...
        if (path.yourTravelTime === 0 && path.eleTravelTime === 0) {
            ...
            yourVisitedRooms.push(path.yourCurrentRoom)
            eleVisitedRooms.push(path.eleCurrentRoom)
            let yourBestRoom = "";
            let eleBestRoom = "";
            ...
            path.roomsToVisit.forEach((_, r) => {
                let remainingTimeAfterMoving =
                    path.time - bestRoomPaths[`${path.yourCurrentRoom},${r}`] - 1;
                // TODO: make sure this function is correct - we should be 
                // not moving to rooms the elephant would be better to moving next
                let eleVal = computeTargetRoomScore(rooms, path, bestRoomPaths, r, false)
                let yourVal = remainingTimeAfterMoving * rooms[r].rate
                if (yourVal > eleVal && yourVal > bestVal) {
            ...
            path.roomsToVisit.forEach((_, r) => {
                ...
                // TODO: same as above
                let yourVal = computeTargetRoomScore(rooms, path, bestRoomPaths, r, true)
                let eleVal = remainingTimeAfterMoving * rooms[r].rate
                if (eleVal > yourVal && eleVal > bestVal) {
                    bestVal = remainingTimeAfterMoving * rooms[r].rate;
                    bestRoom = r;
                }
            });
